...
The preferred artifact maps to the user's explicit output path. Related resources and fragments use safe portable suggested names. Typed manifest resources are validated with the Bundle but remain in staging, so internal layout metadata does not become a user file or collide with later exports. Resolved Markdown-to-DOCX requires exactly one preferred DOCX, one primary entry, and no relations. It has no original-source companion. The Assistant revalidates staged bytes before and after copying and rechecks target identity before commit. Only the confirmed preferred target may be replaced; adjacent historical companion files remain untouched. Commit retains exclusive creation, same-directory backup links, no-clobber links, and rollback without overwriting concurrently created files. DocWen never receives a Vault output path.
