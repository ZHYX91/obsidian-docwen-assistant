...
The preferred artifact maps to the user's explicit output path. Related resources and fragments normally map beside it by safe portable `suggested_name`. Resolved Markdown-to-DOCX is stricter: its Bundle must contain exactly the preferred DOCX and one `application/vnd.docwen.round-trip-sidecar+zip` resource connected by the sole `resource_of(role=manifest, ordinal=0)` relation. The sidecar maps to the explicit DOCX path plus `.docwen`; missing, damaged, extra, or ambiguous resources fail before commit. DocWen creates this single-file resource, and the Assistant never reconstructs its contents. The Assistant revalidates each staged source before and after copying, creates transaction-local files exclusively, and rechecks target identity immediately before commit. Only the explicitly confirmed preferred target may normally be replaced; the exact round-trip sidecar is the one exception and may be replaced together with that DOCX as a single transaction. Other related artifacts are never overwritten. Existing bytes are retained through exclusive same-directory backup links, and prepared outputs are installed with exclusive no-clobber links. Failure removes only transaction-owned unchanged files and restores unchanged backups without overwriting a concurrently created target. DocWen never writes an Obsidian Vault path directly.
