...
插件要求 Windows、Obsidian 1.12.7 或更高版本，以及 Microsoft Store 安装版或完整解压的 ZIP 便携版稳定 DocWen 0.10.x。插件只接受 `docwen.machine.v2` 和 `docwen.artifact_bundle.v2`；其他 Bundle schema 与不兼容的进程信封均失败关闭。
