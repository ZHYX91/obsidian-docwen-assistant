...
`npm run acceptance:docwen-package` 只接受绑定版本、Candidate Bundle v3 身份和哈希的完整 DocWen Windows 包。它验证 Machine 健康、能力、v4 精确双输入、DocWen 1 至 9 级标题、两种当前脚注写法与当前尾注写法、Obsidian 已解析的嵌入资源、Unicode/空格路径、实际 DOCX 图片字节、完整 `docwen.document_node.v1` 目录发布、清单不计入业务输出数量、同一父目录连续导出与 DocWen Bundle v3 写入边界；没有精确候选身份时应跳过或失败关闭，不能转称为源码通过。
