...
describe("release workflow boundary", () => {
  ...
  it("keeps the immutable public DocWen package compatibility gate", () => {
    const compatibilityIndex = verify.indexOf(
      "npm run release:docwen-compatibility",
    );
    const candidateIndex = verify.indexOf("node scripts/release.mjs candidate");

    expect(packageJson.scripts["release:docwen-compatibility"]).toBe(
      "node scripts/check-docwen-compatibility.mjs",
    );
    expect(compatibilityIndex).toBeGreaterThan(-1);
    expect(candidateIndex).toBeGreaterThan(compatibilityIndex);
  });
